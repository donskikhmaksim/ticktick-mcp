...
Режимы:
  --write   переписать якоря в файлах. ВАЖНО: --base это ревизия, на
            которой якоря ДЕЙСТВИТЕЛЬНЫ СЕЙЧАС (для второго и следующих
            прогонов — предыдущая перепривязка), а не начало всей работы;
...
def main() -> int:
    ...
    if args.write:
        # Журнал ПЕРЕЗАПИСЫВАЕТСЯ, а не копится: обратная проверка сверяет
        # текущее состояние документов с текущим деревом. Накопленные записи
        # прошлых прогонов указывали бы на адреса, которые с тех пор уехали
        # ещё раз, и `--verify` кричал бы о расхождениях на пустом месте.
