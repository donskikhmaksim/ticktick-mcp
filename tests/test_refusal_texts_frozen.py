...
EXPECTED = {
    ...
    'add_task_comment/исполнение/trashed': '🛑 НЕ добавил комментарий — задача «Старая затея» лежит В КОРЗИНЕ (удалена): операция над удалённым объектом не выполняется — верните её через apply_task_changes(op=\"restore\"), и тогда повторите. Ничего не тронул.',
    ...
    'add_task_comment/план/trashed': '🛑 План НЕ построен — задача «Старая затея» лежит В КОРЗИНЕ (удалена): операция над удалённым объектом не выполняется — верните её через apply_task_changes(op=\"restore\"), и тогда повторите. Ничего не изменено.',
    ...
    'attach_file_to_task/исполнение/trashed': '🛑 НЕ прикрепил — задача «Старая затея» лежит В КОРЗИНЕ (удалена): операция над удалённым объектом не выполняется — верните её через apply_task_changes(op=\"restore\"), и тогда повторите. Ничего не тронул.',
    ...
    'attach_file_to_task/план/trashed': '🛑 План НЕ построен — задача «Старая затея» лежит В КОРЗИНЕ (удалена): операция над удалённым объектом не выполняется — верните её через apply_task_changes(op=\"restore\"), и тогда повторите. Ничего не изменено.',
    ...
    'delete_task_comment/исполнение/trashed': '🛑 НЕ удалил комментарий — задача «Старая затея» лежит В КОРЗИНЕ (удалена): операция над удалённым объектом не выполняется — верните её через apply_task_changes(op=\"restore\"), и тогда повторите. Ничего не тронул.',
    ...
    'delete_task_comment/план/trashed': '🛑 План НЕ построен — задача «Старая затея» лежит В КОРЗИНЕ (удалена): операция над удалённым объектом не выполняется — верните её через apply_task_changes(op=\"restore\"), и тогда повторите. Ничего не изменено.',
    ...
    'duplicate_task/исполнение/trashed': '🛑 НЕ дублировал — задача «Старая затея» лежит В КОРЗИНЕ (удалена): операция над удалённым объектом не выполняется — верните её через apply_task_changes(op=\"restore\"), и тогда повторите. Ничего не тронул.',
    ...
    'duplicate_task/план/trashed': '🛑 План НЕ построен — задача «Старая затея» лежит В КОРЗИНЕ (удалена): операция над удалённым объектом не выполняется — верните её через apply_task_changes(op=\"restore\"), и тогда повторите. Ничего не изменено.',
    ...
    'update_task_comment/исполнение/trashed': '🛑 НЕ изменил комментарий — задача «Старая затея» лежит В КОРЗИНЕ (удалена): операция над удалённым объектом не выполняется — верните её через apply_task_changes(op=\"restore\"), и тогда повторите. Ничего не тронул.',
    ...
    'update_task_comment/план/trashed': '🛑 План НЕ построен — задача «Старая затея» лежит В КОРЗИНЕ (удалена): операция над удалённым объектом не выполняется — верните её через apply_task_changes(op=\"restore\"), и тогда повторите. Ничего не изменено.',
    ...
    'update_tasks/исполнение/trashed': '🛑 НЕ обновил «Старая затея» — задача «Старая затея» лежит В КОРЗИНЕ (удалена) — верните её через apply_task_changes(op=\"restore\"), прежде чем работать с ней',
}
...
async def test_trashed_task_is_refused_on_every_gated_tool(case, monkeypatch):
    ...
    assert 'apply_task_changes(op="restore")' in got, f"{case}: не сказано, как вернуть:\n{got}"
