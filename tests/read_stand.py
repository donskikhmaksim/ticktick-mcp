...
def direct(tool_name: str):
    """Точка входа инструмента ДО слоя 1 (проверки ключа автоматики).

    2026-08-10, §1.3.4. У тринадцати инструментов, изменяющих задачи, прямой
    путь закрыт: без ключа они возвращают обучающий отказ и ничего не делают.
    Их ФАЗА ПЛАНА при этом никуда не делась — она живой код, которым ходит
    автоматика, и её поведение (сверка личности объекта, политика корзины,
    пометка неприменимых строк, тексты отказов) обязано оставаться под
    проверкой. Позвать эту фазу через ключ нельзя: верный ключ пропускает
    гейт согласия целиком и уводит сразу в исполнение.

    Поэтому предметные тесты зовут вход `.direct`, выставленный декоратором
    `_automation_only`. Это не дыра и не ослабление: сам слой 1 проверяется
    отдельно и по КАЖДОМУ из тринадцати имён — tests/test_hidden_tools.py
    (там вызов идёт через настоящий `mcp.call_tool`, а «ничего не сделано»
    проверяется чтением живого состояния).

    Для инструмента без слоя 1 возвращает его самого — вызывающему не нужно
    знать, закрыт этот конкретный инструмент или нет."""
    fn = getattr(s, tool_name)
    return getattr(fn, "direct", fn)
...
async def call_direct(tool_name: str, /, **args) -> str:
    """То же, что `call`, но ДО слоя 1 (см. `direct` выше).

    Нужна тестам предметного поведения закрытых инструментов: их фаза плана
    жива и ходит по ней автоматика, а через `call` до неё уже не добраться —
    вернётся отказ по ключу. Тесты САМОГО отказа зовут `call`, а не эту
    функцию: иначе они проверяли бы то, что обошли.

    Для инструмента БЕЗ слоя 1 ведёт себя в точности как `call` (тот же
    вызов через реестр, та же валидация схемы) — чтобы в параметризованных
    таблицах, где закрытые и открытые инструменты идут вперемешку, не
    приходилось выбирать функцию по имени."""
    entry = direct(tool_name)
    if entry is getattr(s, tool_name):
