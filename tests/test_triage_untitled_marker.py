...
# Причина в Д3: «·» — это ещё и разделитель мета-полей компактной строки
# (замороженный контракт, по нему разбирает вывод внешний бот). У безымянной
# задачи с вложением и сроком получалось ДВА вхождения « · », и регулярка
# бота разъезжалась: в «название» попадало «(без названия», в «мета» —
# «📎 1 файл) · due …». Строка захардкожена намеренно: она и есть контракт
# вывода, вычислять её из функции сервера — значит проверять тождество.
