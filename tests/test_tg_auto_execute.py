...
def test_auto_executed_tombstone_is_not_overwritten_by_pruning():
    """Причина надгробия несёт ответ модели (_manifest_gone_msg) — затирать её
    общей отметкой при уборке нельзя.

    ОЖИДАНИЕ ОБНОВЛЕНО ПРИ СЛИЯНИИ 2026-08-06, смысл теста прежний. Здесь
    стояло "tg_auto_executed", потому что раньше отметка «исполнено» ставилась
    в момент ЗАХВАТА плана — до самого исполнения. Ветка «трёх тихих отказов»
    это разделила: захват помечается `_TOMBSTONE_CLAIMED` («нажато, исход
    неизвестен»), а «выполнено»/«НЕ выполнено» пишется уже по факту, в
    `_tg_auto_execute_tick`. Проверяем ровно то же самое свойство — уборка не
    трогает причину, — но на честной причине этой фазы."""
    s._MANIFESTS["btn-1"] = {"kind": "delete", "consumed": False,
                             "created": time.monotonic()}
    s._consume_manifest_for_auto_execute("btn-1")
    s._prune_manifests()
    assert s._MANIFEST_TOMBSTONES["btn-1"]["reason"] == s._TOMBSTONE_CLAIMED
