...
# `rg -c "^@mcp\.tool\(" ticktick_mcp/src/server.py` был 77 на ревизии-якоре
# этого документа. 2026-08-09 (П15 п.4, 1.3.5): добавлена 78-я команда,
# `find_untitled_tasks` — разобрана отдельной строкой таблицы, число сверки
# сдвинуто вместе с ней, а не подогнано мимо таблицы.
# 77 → 78 (2026-08-09, П20/ZAHOD1.md §1.3.6): +1 = `delete_tags`
# — строка добавлена в docs/TZ/inventar_opisaniy.md отдельно, см. её
# addendum-примечание там же.
# Обе прибавки — РАЗНЫМИ ветками от одной базы (77), верный после слияния
# только их суммарный итог: 77 + 1 + 1 = 79.
# 79 → 80 (2026-08-10, §1.3.4 шаг 1): +1 = `manual_triage` — псевдоним
# переименованного `apply_task_changes`, зарегистрированный отдельной
# командой (см. отдельную строку в таблице).
_EXPECTED_TOOL_COUNT = 80
...
def _registered_tool_names():
    ...
    2026-08-10 (§1.3.4, шаг 4): источник — сам РЕЕСТР FastMCP, а не
    `list_tools()`. Листинг с этого дня фильтруется (часть команд намеренно не
    публикуется, оставаясь вызываемыми по имени), а инвентаризация описаний
    обязана покрывать ВСЕ зарегистрированные команды, включая спрятанные:
    их докстринг читает тот, кто зовёт их из автоматики."""
