...
@pytest.fixture(autouse=True)
def relay_token_journal(monkeypatch, tmp_path):
    """17.3: /dl and /ul now durably record each consumed jti under
    _JOURNAL_DIR (default '/data', a Railway volume — not writable/desired in
    tests) so a token can only ever be used once. Point it at a per-test
    tmp_path instead."""
