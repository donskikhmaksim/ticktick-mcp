...
def _wire_restore(monkeypatch, reads, trash):
    ...
    class _FakeV2Restore:
        def get_trash(self, limit=500):
            return list(trash.values())[:limit]
        ...
        def find_task_any_state(self, task_id):
            # Добавлено 2026-08-07 вместе с фиксом «восстановление рапортует
            # „не восстановлено" об успехе»: пост-проверка, не найдя задачу
            # среди ОТКРЫТЫХ, больше не выносит приговор сразу — она
            # спрашивает, где задача на самом деле (завершённая возвращается
            # из корзины завершённой и в открытых не появляется никогда).
            # Двойник отвечает по СВОЕМУ же состоянию: ленты завершённых у
            # него нет, а корзину он держит — значит «всё ещё в корзине»
            # (реальный провал) он назвать может, и именно это здесь и
            # проверяется.
            entry = trash.get(task_id)
            return (entry, "trash") if entry else (None, None)
