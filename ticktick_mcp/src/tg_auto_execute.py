...
Вынесено из `server.py` ДОСЛОВНО — строки 16125–18016 на ревизии 0b3c04c62c93
с учётом коммита 1; ни одного изменённого символа, совпадение доказывается
контрольной суммой (`tools/verify_move.sh`), а не чтением.

Почему отдельным файлом. Это единственный путь, по которому сервер УДАЛЯЕТ
задачи владельца без участия модели: владелец жмёт кнопку, поллер находит
одобренный манифест и зовёт исполнителя напрямую, минуя слой команд MCP.
Связь с `server.py` двусторонняя и разрешается позицией импорта: тот
импортирует этот модуль там, где раньше лежал сам кусок, — то есть после
того, как определены все имена, которые кусок берёт снаружи.
"""
import asyncio
import json
import os
...
# Пространство имён главного файла целиком — не для удобства: все 32 функции
# `_<тул>_impl` остались там. До 1.2.4 generic-исполнитель искал их через
# `globals()`, и это работало, пока код лежал с ними в одном файле. После
# переезда `globals()` здесь — словарь ЭТОГО модуля: поиск вернул бы None для
# КАЖДОЙ из 30 команд под `_gate_single`/`_gate_batch`, а на живом сервере это
# «нажал кнопку, ничего не произошло», без единой ошибки в журнале. Поэтому
# обращение явное — и через модуль, а не `from .server import ...`, чтобы
...
def _tg_auto_execute_approved(pending: List[tuple],
    ...
    for mid, tool in pending:
        ...
        out.append({"manifest_id": mid, "tool": tool,
                    ...
                    # Момент нажатия кнопки (ставит вебхук). Нужен ровно для
                    # одного: посчитать и записать в лог, сколько нажатие
                    # прождало исполнения, — ночной QA 2026-08-19 дважды
                    # принял 14-минутную очередь за сломанную кнопку, потому
                    # что из логов эту задержку было не увидеть.
...
# Сколько подтверждённых кандидатов ОДНОГО тика исполняются одновременно.
# Появился из ночного инцидента 2026-08-19: кандидаты шли строго по одному,
# и на каждого приходились не только сама мутация, но и независимая
# перепроверка плюс публикация отчёта в Telegram — а Telegram под активным QA
# отвечает 429 и заставляет отсиживать retry_after (до _MAX_SEND_WAIT_S на
# кусок). Нажатая кнопка ждала, пока ОТЧИТАЮТСЯ все предыдущие: пять
# параллельных сессий превращали 10-секундный тик в ~14 минут. Потолок, а не
# безлимит: каждый кандидат занимает поток пула (_run_blocking) на время
# своих синхронных вызовов, и десяток одновременных публикаций съел бы пул у
# обычных MCP-запросов.
_TG_AUTO_EXECUTE_MAX_PARALLEL = max(1, int(
    os.environ.get("TG_AUTO_EXECUTE_MAX_PARALLEL", "4")))
...
async def _run_one_auto_execute_candidate(c: Dict) -> None:
    """Полный цикл ОДНОГО подтверждённого кандидата: атомарный захват
    плана, исполнение, независимая перепроверка, надгробие по исходу и
    публикация отчёта. Дословно бывшее тело цикла из
    `_tg_auto_execute_tick`, вынесенное отдельной корутиной 2026-08-19,
    чтобы кандидатов можно было исполнять ПАРАЛЛЕЛЬНО (см. тик):
    последовательный обход заставлял нажатую кнопку ждать, пока про всех
    предыдущих кандидатов ОТЧИТАЮТСЯ в задушенный флуд-лимитом Telegram.

    Никогда не бросает по существу: любое исключение исполнения ловится
    внутри и превращается в отчёт об ошибке (инвариант «упавший кандидат
    не рушит остальных» теперь держится ещё и на изоляции задач gather).

    Про гонки при параллельном запуске — почему их нет:
      * одноразовость захвата — SQL `UPDATE … WHERE consumed_at IS NULL`
        (`_consume_manifest_for_auto_execute`), два кандидата с одним
        manifest_id невозможны и внутри списка (`rows` — dict);
      * `_TG_AUTO_EXECUTE_MANIFEST` — contextvar: gather запускает каждую
        корутину отдельной задачей со СВОЕЙ копией контекста, а
        `asyncio.to_thread` копирует контекст в поток — метка манифеста
        не протекает между кандидатами;
      * `_MANIFESTS`/надгробия правятся точечными операциями dict,
        атомарными под GIL, — как и раньше, когда часть их шла из
        рабочего потока."""
    ...
    try:
        ...
        if consumed is None:
            return  # race/drift/already consumed — not an error
        # Наблюдаемость (2026-08-19): сколько нажатие ПРОЖДАЛО исполнения.
        # Ночью QA дважды счёл рабочий механизм сломанным, потому что
        # 14-минутная очередь в логах выглядела как полное молчание, —
        # теперь задержка названа числом прямо в момент старта исполнения.
        decided_at = c.get("decided_at")
        if decided_at:
            waited_s = max(0.0, time.time() - decided_at / 1000.0)
            logger.info(f"TG auto-execute: {tool}/{mid}: исполнение началось "
                        f"через {waited_s:.1f} c после нажатия кнопки")
...
async def _tg_auto_execute_tick() -> None:
    ...
    candidates = _tg_auto_execute_approved(pending, rows)
    if candidates:
        exec_started = time.monotonic()
        # ПАРАЛЛЕЛЬНО, а не по одному (2026-08-19). Ночной QA пятью
        # сессиями показал: при последовательном обходе задержка «кнопка →
        # исполнение» растёт ЛИНЕЙНО с числом подтверждённых кандидатов,
        # потому что на каждого приходится не только мутация, но и
        # перепроверка + публикация отчёта в Telegram (со снами на 429 до
        # `_MAX_SEND_WAIT_S` на кусок) — доходило до ~14 минут при тике в
        # 10 секунд. Семафор ограничивает одновременность, чтобы не съесть
        # пул потоков `_run_blocking` у обычных MCP-запросов.
        sem = asyncio.Semaphore(_TG_AUTO_EXECUTE_MAX_PARALLEL)

        async def _guarded(cand: Dict) -> None:
            async with sem:
                await _run_one_auto_execute_candidate(cand)

        # return_exceptions=True — страховка: тело кандидата и так ловит
        # всё своё, но неожиданное исключение одной задачи не должно ни
        # оборвать соседние, ни унести весь тик мимо lost-ветки ниже.
        results = await asyncio.gather(
            *(_guarded(c) for c in candidates), return_exceptions=True)
        for cand, res in zip(candidates, results):
            if isinstance(res, BaseException):
                logger.error(
                    "TG auto-execute: кандидат "
                    f"{cand.get('tool')}/{cand.get('manifest_id')} упал вне "
                    f"собственного try/except: {res!r}")
        # Наблюдаемость (2026-08-19): одна строка на тик с кандидатами —
        # по ней в логах видно и размер очереди, и стоимость прохода,
        # чтобы «тихие 14 минут» больше не выглядели сломанной кнопкой.
        logger.info(
            f"TG auto-execute: тик исполнил {len(candidates)} кандидатов "
            f"за {time.monotonic() - exec_started:.1f} c "
            f"(живых планов: {len(pending)}, "
            f"параллельность: {_TG_AUTO_EXECUTE_MAX_PARALLEL})")

    # Исчезновение сообщений гейта (2026-08-10, TZ_temp_automation_key.md
    # §5) — best-effort уборка того, что накопил `tg_approval.
    # schedule_message_delete`: план/кнопки после ответа, отчёт сразу после
    # отправки. С 2026-08-19 идёт ПОСЛЕ исполнения кандидатов, а не до:
    # уборка — это отдельный HTTP `deleteMessage` на КАЖДОЕ накопившееся
    # сообщение, и, стоя первой, она линейно оттягивала исполнение уже
    # нажатой кнопки (активная сессия плодит десятки сообщений на тик).
    # Удаление и так отложено на ~60 секунд — ещё несколько секунд в конце
    # тика ему безразличны, а кнопке — нет. `_run_blocking` — тем же
    # принципом, что и остальная синхронная сеть в этом файле; сбой ОДНОГО
    # удаления не прерывает ни уборку, ни остальной тик.
    try:
        ...
        # роняет остальной тик (lost-ветка ниже)
