...
def _log_gate_bypass(action: str, tool: str = "") -> None:
    ...
    Формулировка — «ДОПУЩЕНО к исполнению», не «выполнено» (QA-2 2026-08-19,
    добор №8): строка пишется ДО исполнения, а дальше по пути ещё возможен
    отказ (протухший манифест, пустой список, identity guard) — «выполнено»
    в этих случаях было бы ложью в аудит-логе."""
    logger.warning(
        f"🔓 ГЕЙТ ВЫКЛЮЧЕН переключателем {_GATE_DISABLED_ENV}: действие "
        f"'{action}' (tool={tool or '?'}) допущено к исполнению БЕЗ "
        "подтверждения пользователя (итог исполнения — в журнале мутаций).")
...
def _require_consent(
    *, action: str, tier: int, manifest: Optional[Dict] = None,
    ...
    ВНИМАНИЕ (слияние 2026-08-19): аварийный выключатель `_gate_disabled()`
    сюда НЕ возвращать. Его место — ниже, после механики манифеста и после
    явного «нет» человека (разбор QA-2); в ветке про блокировки loop'а он
    оставался наверху лишь потому, что она отпочковалась раньше того фикса."""
...
def _prune_manifests() -> None:
    ...
    for mid in [m for m, v in _MANIFESTS.items()
        ...
        # ПЛАН, ПО КОТОРОМУ ПРЯМО СЕЙЧАС ИДЁТ СИНХРОННОЕ ОЖИДАНИЕ, НЕ
        # ТРОГАЕМ (2026-08-14, баг повторных запросов). `_sync_wait_for_
        # decision` — единственный читатель, которому исход решения виден
        # ТОЛЬКО в самом манифесте (`_web_decision`/`_web_result`): в базе
        # этих полей нет, надгробие несёт лишь причину. Пока запись жила без
        # этой оговорки, порядок был такой: владелец подтверждает в веб-
        # портале → манифест погашен → портал дёргает GET /pending-consents →
        # тот зовёт `_prune_manifests()` → запись исчезает → ожидание видит
        # None, крутится до конца таймаута и возвращает ОБЫЧНОЕ превью плана
        # с приглашением «вызови инструмент снова» — и модель шла просить
        # подтверждение по второму кругу. Задержка удаления ограничена окном
        # ожидания (`CONSENT_SYNC_WAIT_MS`, секунды): следующий же prune
        # после его конца уберёт запись как обычно.
        if (_MANIFESTS.get(mid) or {}).get("_sync_waiters"):
            continue
...
_DECIDED_ELSEWHERE_MSG = (
    "✅ Решение по этому плану уже принято, пока шло ожидание ответа — план "
    "погашен и больше не активен. Повторно вызывать этот инструмент с этим "
    "планом НЕ нужно и повторно спрашивать пользователя тоже не надо: "
    "фактический результат посмотри через operation_report.")

_WEB_IN_FLIGHT_MSG = (
    "⏳ Пользователь подтвердил этот план в веб-портале подтверждений, и "
    "сервер ПРЯМО СЕЙЧАС его исполняет. Повторно вызывать этот инструмент с "
    "этим планом не нужно и подтверждение переспрашивать не надо — результат "
    "смотри через operation_report.")

_WEB_CONFIRMED_NO_REPORT_MSG = (
    "✅ Подтверждено и исполнено. Решение по этому плану уже принято и "
    "исполнено через веб-подтверждение, пока шло ожидание ответа. Повторно "
    "вызывать этот инструмент с этим планом не нужно.")

_DECIDED_OTHER_CHANNEL_MSG = (
...
def _sync_wait_hold(manifest_id: str, delta: int) -> None:
    """Счётчик «по этому плану идёт синхронное ожидание» (2026-08-14).

    Счётчик, а не булев флаг: теоретически ожиданий по одному плану может
    оказаться два (ретрай вызова инструмента, пока первый ещё висит), и
    выход одного из них не должен снимать защиту у второго. Ключ живёт в
    самом манифесте — там же, где `consumed`, — потому что защищаемое место
    (`_prune_manifests`) видит именно манифест и ничего больше."""
    m = _MANIFESTS.get(manifest_id)
    if m is None:
        return
    left = int(m.get("_sync_waiters") or 0) + delta
    if left > 0:
        m["_sync_waiters"] = left
    else:
        m.pop("_sync_waiters", None)


def _sync_wait_decision_text(m: Dict) -> Optional[str]:
    """Текст исхода для ПОГАШЕННОГО манифеста, либо None — «исход ещё не
    дописан, подожди» (веб-канал погасил план и исполняет его прямо сейчас).

    None возвращается только в одном случае: решение confirm пришло с
    веб-канала, `_web_in_flight` ещё стоит, а `_web_result` ещё не записан.
    Отдавать в этот момент обобщённое «решение принято» — значит терять
    настоящий отчёт об исполнении, ради которого ожидание и заведено."""
    decision = m.get("_web_decision")
    if decision == "confirmed":
        result = m.get("_web_result")
        if result:
            return result
        if m.get("_web_in_flight"):
            return None
        return _WEB_CONFIRMED_NO_REPORT_MSG
    ...
    return _DECIDED_OTHER_CHANNEL_MSG
...
async def _sync_wait_for_decision(manifest_id: str) -> Optional[str]:
    ...
    # Пока крутится этот цикл, `_prune_manifests` не имеет права выдернуть
    # запись у нас из-под ног (см. там же) — иначе исход решения читать будет
    # неоткуда: `_web_decision`/`_web_result` живут ТОЛЬКО в манифесте.
    _sync_wait_hold(manifest_id, +1)
    # Исход, известный НЕ ПОЛНОСТЬЮ: план подтверждён, исполнение идёт прямо
    # сейчас, отчёт ещё не дописан. Если ждать до конца окна и не дождаться —
    # возвращаем ЭТО, а не None: None означает «никто ничего не решил» и
    # печатает превью плана, то есть просит подтвердить уже подтверждённое.
    ...
    try:
        while time.monotonic() < deadline:
            ...
            m = _MANIFESTS.get(manifest_id)
            if m is None:
                # ЗАПИСИ БОЛЬШЕ НЕТ = РЕШЕНИЕ ПРИНЯТО (2026-08-14). Так и
                # написано в исходном ТЗ веб-хаба: `if (!row || row.status
                # !== "AWAITING_CONSENT") break;`. Здесь до этой правки стоял
                # `continue`, и питон-порт крутился до конца окна, а затем
                # возвращал превью плана «вызови инструмент снова» — по
                # погашенному плану. Живой манифест из-под ожидания больше не
                # исчезает (см. `_sync_wait_hold`), так что сюда мы попадаем
                # только в аварийном порядке: план погасил и удалил кто-то
                # мимо этой защиты. Исход берём из надгробия (`_manifest_gone_
                # msg` различает исполнено / исполняется / провалено /
                # подтверждено не всё); нет надгробия — честный общий текст,
                # но НИКОГДА не превью.
                return _manifest_gone_msg(manifest_id, _DECIDED_ELSEWHERE_MSG)
            if not m.get("consumed"):
                continue
            decided = _sync_wait_decision_text(m)
            if decided is not None:
                return decided
            # Веб-канал уже погасил план и прямо сейчас его исполняет, но
            # отчёта ещё нет — ждём его до конца окна.
            pending = _WEB_IN_FLIGHT_MSG
    finally:
        _sync_wait_hold(manifest_id, -1)
    return pending
...
async def _gate_batch(kind: str, tool_name: str, tasks: Optional[List[Dict]],
    ...
    # контексте ЭТОЙ корутины (см. её докстринг). Считаем его только в ветке
    # без отказа: при «нет» канал не нужен, и лишнего похода в базу не будет.
